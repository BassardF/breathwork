...
export function SafetyDisclaimer() {
  ...
  const setSafetyAcknowledged = useAuthStore(
    (state) => state.setSafetyAcknowledged,
  );
      ...
      <Card className="max-w-2xl space-y-6">
        <div className="space-y-3">
          ...
          <h1 className="text-4xl font-semibold tracking-tight text-white">
            Acknowledge before training
          </h1>
          <p className="text-lg leading-8 text-slate-300">
            Breath hold training carries risk of hypoxic blackout. Never
            practice alone or in/near water without a trained buddy. This app is
            designed for dry land training only.
          </p>
        </div>
        <Button
          fullWidth
          onClick={() => {
            if (isLocalMode) {
              ...
              void updateSafetyAcknowledgement().then(() => {
