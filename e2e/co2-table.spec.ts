...
test.beforeEach(async ({ page }) => {
  ...
  await page.goto(navUrl('/co2'));
});

test('renders CO2 table setup with sliders and preview', async ({ page }) => {
  await expect(page.getByRole('heading', { name: 'CO2 Table' })).toBeVisible();
  await expect(page.getByText('Fixed hold. Falling rest.')).toBeVisible();
...
test('starts a CO2 session and shows rest phase', async ({ page }) => {
...
test('ends a CO2 session early', async ({ page }) => {
  await page.getByRole('button', { name: 'Start session' }).click();
  await expect(page.getByText('rest phase')).toBeVisible({ timeout: 5000 });
...
test('shows info and tips section', async ({ page }) => {
  ...
  await expect(page.getByText(/carbon dioxide/i)).toBeVisible();
